...
<section data-view="feed" class="on" id="view-feed">
  <div class="wrap feedpage">
    ...
    <!-- AI 자동 동향분석: 숫자·키워드는 스크립트, 문장은 검토 때 작성 (content/feed.json insight) -->
    <section class="insight" id="insight" aria-label="AI 자동 동향분석">
      <div>
        <div class="ttl"><i>AI</i>AI 자동 동향분석 <span class="method" id="ins-date" style="margin:0 0 0 6px"></span></div>
        <h2 id="ins-headline">오늘 수집분을 아직 분석하지 않았습니다.</h2>
        <ol id="ins-points"></ol>
        <div class="next" id="ins-next" hidden></div>
        <p class="method" id="ins-method"></p>
      </div>
      <div class="istats" id="ins-stats"></div>
    </section>

    <div id="rows"></div>

    <details class="listtoggle" id="listtoggle">
      <summary>전체 목록과 필터 <span class="rowhead"><span class="n" id="list-count" style="font-family:var(--mono);font-size:13px;color:var(--mut);font-weight:400"></span></span></summary>
      <div class="feedwrap">
        ...
        <div>
          <div class="feedhead">
            <h1 id="feed-title" style="font-size:22px">모인 것 전체</h1>
...
(function(){
  ...
  const qv=new URLSearchParams(location.search).get('view');const h=location.hash.replace('#','');go(views.includes(h)?h:(views.includes(qv)?qv:'feed'),false);
...
function renderRows(F){
  ...
  const card=i=>{
    const chips=[];
    if(i.channel==='jobs'){(i.category||'').split(' · ').filter(Boolean).forEach(c=>chips.push('<span>'+esc(c)+'</span>'));(i.tags||[]).slice(0,2).forEach(t=>chips.push('<span>'+esc(t)+'</span>'));}
    else{if(i.category)chips.push('<span>'+esc(i.category)+'</span>');if(i.relevance==='high')chips.push('<span class="hi">중요</span>');if(i.impact)chips.push('<span class="imp-'+esc(i.impact)+'">'+({strengthen:'전망 강화',weaken:'전망 약화',neutral:'중립'}[i.impact]||i.impact)+'</span>');if(i.official===false)chips.push('<span class="hi">비공식</span>');(i.tags||[]).slice(0,2).forEach(t=>t&&chips.push('<span>'+esc(t)+'</span>'));}
    const one=i.channel==='jobs'?(i.summary||''):(i.note?'<b>나에게</b>'+esc(i.note):esc(i.summary||''));
    const title=i.url?'<a href="'+esc(i.url)+'" target="_blank" rel="noopener noreferrer">'+esc(i.title)+'</a>':esc(i.title);
    return '<article class="cd '+(i.relevance==='high'?'high':'')+'"><h3>'+title+'</h3><div class="org">'+esc(i.publisher)+dd(i)+'<span class="dt">'+esc(i.published_at||'')+'</span></div><div class="chips">'+chips.join('')+'</div><p class="one">'+(i.channel==='jobs'?esc(one):one)+'</p></article>';
  };
  const secs=[
    {id:'jobs',title:'오늘의 채용 공고',sub:'신입·경력무관 · 마감 가까운 순',pick:()=>F.items.filter(i=>i.channel==='jobs'&&(i.dday==null||i.dday>=0)).sort((a,b)=>(a.dday??999)-(b.dday??999)).slice(0,12)},
    {id:'health',title:'보건·의료에서 바뀌는 것',sub:'공식 출처 · 검토된 항목만',pick:()=>F.items.filter(i=>i.channel==='health'||i.channel==='research').slice(0,12)},
    {id:'ai',title:'AI 신호',sub:'전망에 반영된 공식 자료',pick:()=>F.items.filter(i=>i.channel==='ai').slice(0,12)},
    {id:'sql',title:'오늘의 SQL 5문제',sub:'원문 3 + 생성 2',pick:()=>F.items.filter(i=>i.channel==='sql')}
  ];
  rows.innerHTML=secs.map(sec=>{const it=sec.pick();if(!it.length)return '';
    return '<div class="rowhead"><h2>'+esc(sec.title)+' <span class="n">'+it.length+'</span></h2><span class="sub">'+esc(sec.sub)+'</span><a href="#list" data-ch="'+sec.id+'">전체 보기 →</a></div>'
      +'<div class="rowscroll"><button class="arrow l" aria-label="이전">‹</button><div class="cards">'+it.map(card).join('')+'<article class="cd more"><a href="#list" data-ch="'+sec.id+'">'+esc(CH[sec.id]||sec.id)+' 전체 →</a></article></div><button class="arrow r" aria-label="다음">›</button></div>';}).join('');
  rows.querySelectorAll('.rowscroll').forEach(r=>{const c=r.querySelector('.cards');const l=r.querySelector('.arrow.l'),rt=r.querySelector('.arrow.r');
    const upd=()=>{l.disabled=c.scrollLeft<=2;rt.disabled=c.scrollLeft+c.clientWidth>=c.scrollWidth-2;};
    l.addEventListener('click',()=>c.scrollBy({left:-c.clientWidth*.8}));rt.addEventListener('click',()=>c.scrollBy({left:c.clientWidth*.8}));c.addEventListener('scroll',upd);upd();});
  rows.addEventListener('click',e=>{const a=e.target.closest('a[data-ch]');if(!a)return;e.preventDefault();const d=document.getElementById('listtoggle');d.open=true;
    const li=document.querySelector('#f-channel li[data-v="'+a.dataset.ch+'"]');if(li)li.click();d.scrollIntoView({behavior:'smooth',block:'start'});});
  // 동향분석
  const I=F.insight||{};const $=id=>document.getElementById(id);
  if($('ins-headline')&&I.headline){$('ins-headline').textContent=I.headline;$('ins-points').innerHTML=(I.points||[]).map(p=>'<li>'+esc(p)+'</li>').join('');
    ...
    $('ins-method').textContent=I.method||'';$('ins-date').textContent=I.text_date?'분석 '+I.text_date:'';}
  else if($('ins-method')){$('ins-method').textContent=I.method||'';}
  if($('ins-stats')){const nb=I.new_by_channel||{};
    $('ins-stats').innerHTML='<div class="istat"><div class="k">오늘 신규</div><div class="v">'+esc(F.meta&&F.meta.today_new||0)+'<small>건</small></div></div>'
      +'<div class="istat"><div class="k">7일 내 마감 공고</div><div class="v">'+esc(I.deadline_7d||0)+'<small>건</small></div></div>'
      +'<div class="istat"><div class="k">보건·의료 신규</div><div class="v">'+esc((nb.health||0)+(nb.research||0))+'<small>건</small></div></div>'
      +'<div class="istat"><div class="k">중요 표시</div><div class="v">'+esc(I.high||0)+'<small>건</small></div></div>'
      +'<div class="kws">'+(I.keywords||[]).map(k=>'<span>'+esc(k.k)+'<small>'+esc(k.n)+'</small></span>').join('')+'</div>';}
  const sb=document.getElementById('statusbar');if(sb&&F.pipelines){sb.insertAdjacentHTML('beforeend','');}
  const lc=document.getElementById('list-count');if(lc)lc.textContent=F.items.length+'건';
}
</script>
</body>
</html>
