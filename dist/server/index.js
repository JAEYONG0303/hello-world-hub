...
.rs-quick{list-style:none;margin:18px 0 0;padding:0;display:grid;grid-template-columns:repeat(4,1fr);gap:12px}
...
@media(max-width:900px){.pw{grid-template-columns:1fr;border:0}.rail{position:static;max-height:none;border-right:0;border-bottom:1px solid var(--grid);padding:10px 0}
 ...
 /* 채용 카드는 마감·제목·태그 3칸 구조라 위 규칙과 다르게 카드형으로 짠다 (표처럼 보이지 않게) */
 .feed-row-main.jobs{grid-template-columns:56px minmax(0,1fr);grid-template-rows:auto auto}
 .feed-row-main.jobs>:nth-child(1){grid-row:1;grid-column:1}
 .feed-row-main.jobs>:nth-child(2){grid-row:1;grid-column:2}
 .feed-row-main.jobs>:nth-child(3){grid-row:2;grid-column:1/3;display:flex;flex-wrap:wrap;gap:5px;margin-top:2px}
...
<div class="wrap">
  <div class="rs-hero">
    ...
    <ul class="rs-quick">
      <li><b>해결하려던 문제</b>채용 공고·보건의료 정책·AI 신호를 매일 손으로 찾아보던 일</li>
      <li><b>자동화한 업무</b>수집 → 필터 → 태깅 → 집계까지 스크립트가 매일 처리</li>
      <li><b>현재 결과</b>아래 숫자(채용·감시·신호·문제)로 매일 갱신</li>
      <li><b>검증·사람의 판단</b>발표일 없는 자료 제외, 확률 변경 상한, 최종 승인은 사람이</li>
    </ul>
...
function renderRows(F){
  ...
  const jcard=i=>{const dl=i.dday==null?'<span class="fd-date">'+esc((i.deadline||'').slice(5))+'</span>':'<span class="fd-dd'+(i.dday<=3?' hot':'')+'">'+(i.dday<0?'마감':i.dday===0?'D-day':'D-'+i.dday)+'</span>';
    ...
    const cond=[i.employment,i.experience].filter(Boolean).join('·');
    const cats=[i.job_family,cond,SRC[i.source]||i.source].filter(Boolean);
    return '<div class="feed-row"><div class="feed-row-main jobs">'+dl
      +'<span class="fd-title" title="'+esc(i.title)+'"><a href="'+esc(i.url)+'" target="_blank" rel="noopener noreferrer">'+esc(i.title)+'</a><small class="fd-sub">'+esc(org)+' · '+esc(i.publisher)+'</small></span>'
      +'<span class="fd-cats">'+cats.map(c=>'<i class="fd-cat">'+esc(c)+'</i>').join('')+'</span></div>'
  ...
  const drawChips=()=>{FK.forEach(k=>{const box=document.querySelector('#ftool .fchips[data-k="'+k+'"]');if(!box)return;
      ...
      box.innerHTML='<button data-v="" class="fall '+(st[k]?'':'on')+'">전체 <em>'+base.length+'</em></button>'+keys.map(v=>'<button data-v="'+esc(v)+'" class="'+(st[k]===v?'on':'')+'">'+esc(k==='source'?(SRC[v]||v):v)+' <em>'+(c[v]||0)+'</em></button>').join('');});
    drawTags();};
  const FLABEL={region:'지역',org_type:'기관',job_family:'직종',experience:'경력',employment:'고용형태',source:'출처'};
