...
<script id="hub-stats" type="application/json">{"updated":"2026-09-14","jobs":{"total":64,"upcoming7":28,"next_deadline":"2026-09-14","sources_count":5,"by_source":{"사람인":21,"잡알리오":20,"잡코리아":16,"경제·인문사회연구회":4,"메디컬잡":3},"last_fetched_at":"2026-09-10","new_this_week":64},"fit":{"high":6,"medium":49,"low":6,"review":3},"pipeline":{"collected":162,"dedup_removed":38,"career_excluded":35,"job_excluded":null,"auto_classified":48,"review_needed":29,"final_exposed":64,"reconciled":false,"reconcile_diff":3,"sources":{"ok":5,"fail":0,"skip":1},"run_days":2,"run_dates":["2026-09-09","2026-09-10"],"success_rate":null,"avg_duration_ms":null,"last_success_at":null,"success_streak_days":null,"by_source_success_rate":{},"sample_days":0,"sample_ready":false},"sql":{"bank":36,"engines":{"bigquery":18,"mariadb":18},"solved":0,"correct":0,"accuracy":null,"days":0},"star":{"cards":5,"confirmed":0,"open_items":12},"tracker":{"total":0,"by_status":{}},"note":"my agent 저장소의 CSV·YAML·MD 에서 센 값. 추정치 없음."}</script>
...
function renderRows(F){
  ...
  if($('ftool')){$('ftool').addEventListener('click',e=>{const b=e.target.closest('.fchips button');if(!b)return;const k=b.parentElement.dataset.k;st[k]=b.dataset.v;jpage=1;drawJobs();});
    ...
    $('f-reset').addEventListener('click',()=>{FK.forEach(k=>st[k]='');st.q='';st.soon=false;st.watch=false;st.hideExcluded=true;$('f-q').value='';$('f-soon').classList.remove('on');$('f-soon').setAttribute('aria-pressed','false');$('f-hide-excl').classList.add('on');$('f-hide-excl').setAttribute('aria-pressed','true');jpage=1;drawJobs();});
    ...
    $('jobgrid').addEventListener('change',e=>{const sel=e.target.closest('.jc-status');if(!sel)return;JobState.set(sel.dataset.id,{status:sel.value});jpage=1;drawJobs();});
    $('jobgrid').addEventListener('blur',e=>{const ta=e.target.closest&&e.target.closest('.jc-note');if(!ta)return;JobState.set(ta.dataset.id,{note:ta.value});},true);
    if($('jreset-personal'))$('jreset-personal').addEventListener('click',()=>{
      if(!confirm('저장된 모든 지원상태·메모를 삭제합니다. 되돌릴 수 없습니다. 계속할까요?'))return;
      JobState.resetAll();drawJobs();});
