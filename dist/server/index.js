...
function renderRows(F){
  ...
  if($('winst')){$('winst').addEventListener('click',e=>{const t=e.target.closest('.wtog');if(t){const n=t.dataset.name;const w=WI.find(x=>x.name===n);wOff[n]=wOn(w);try{localStorage.setItem('hub.watch.off',JSON.stringify(wOff));}catch(x){}drawWatch();if(st.watch){jpage=1;drawJobs();}return;}
