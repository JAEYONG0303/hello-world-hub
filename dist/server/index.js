...
<section data-view="feed" class="on" id="view-feed">
  <div class="wrap feedpage">
    ...
    <!-- v5: 잡코리아 메인 구조 (측정값: 2:1 상단 행, 카테고리 칩 + 4열, 3열, 5열) -->
    <div class="jk">
      <div class="jkrow">
        <div class="jkbox" id="insight">
          <div class="t"><b><i>AI</i>AI 자동 동향분석 <span class="meta" id="ins-date"></span></b><a href="#list" data-ch="health">전체 보기 ›</a></div>
          <div class="desc" id="ins-method">숫자·키워드는 스크립트가 세고, 문장은 검토할 때 씁니다.</div>
          <h2 class="hl" id="ins-headline">오늘 수집분을 아직 분석하지 않았습니다.</h2>
          <ol id="ins-points"></ol>
          <div class="nx" id="ins-next" hidden></div>
        </div>
        <div class="jkbox">
          <div class="t"><b>나의 상태</b><a href="#ops">운영판 ›</a></div>
          <div class="me" id="ins-stats"></div>
        </div>
      </div>
    </div>
    <div id="rows"></div>
...
function renderRows(F){
  ...
  const card=i=>{const tg=[];
    if(i.channel==='jobs'){(i.category||'').split(' · ').filter(Boolean).forEach(c=>tg.push('<span>'+esc(c)+'</span>'));}
    else{if(i.category)tg.push('<span>'+esc(i.category)+'</span>');if(i.relevance==='high')tg.push('<span class="hi">중요</span>');if(i.impact)tg.push('<span class="imp-'+esc(i.impact)+'">'+({strengthen:'전망 강화',weaken:'전망 약화',neutral:'중립'}[i.impact]||i.impact)+'</span>');if(i.official===false)tg.push('<span class="hi">비공식</span>');(i.tags||[]).slice(0,1).forEach(t=>t&&tg.push('<span>'+esc(t)+'</span>'));}
    const title=i.url?'<a href="'+esc(i.url)+'" target="_blank" rel="noopener noreferrer">'+esc(i.title)+'</a>':esc(i.title);
    const sm=i.channel==='jobs'?'':(i.note?'<div class="sm">'+esc(i.note)+'</div>':'');
    return '<article class="jc '+(i.relevance==='high'&&i.channel!=='ai'?'high':'')+'"><span class="star">☆</span><h3>'+title+'</h3><div class="co">'+esc(i.publisher)+dd(i)+'<span class="dt">'+esc(i.published_at||'')+'</span></div>'+sm+'<div class="tg">'+tg.join('')+'</div></article>';};
  const fill=(items,n)=>{const out=items.slice(0,n).map(card);while(out.length<n)out.push('<article class="jc empty">—</article>');return out.join('');};
  ...
  const health=F.items.filter(i=>i.channel==='health'||i.channel==='research');
  const ai=F.items.filter(i=>i.channel==='ai');const sql=F.items.filter(i=>i.channel==='sql');
  rows.innerHTML=
    '<section class="jksec"><div class="jkhead"><h2>채용 공고 <span class="n">신입·경력무관 '+jobs.length+'건 · 마감 가까운 순</span></h2><div class="more"><a href="#list" data-ch="jobs">더보기 ›</a></div></div>'
    +'<div class="chips" id="jobfam">'+JOBFAM.map(([k,n,f])=>'<button data-k="'+k+'" class="'+(k==='all'?'on':'')+'">'+n+' '+jobs.filter(f).length+'</button>').join('')+'</div>'
    +'<div class="jk-grid c4" id="jobgrid">'+fill(jobs,8)+'</div></section>'
    +'<section class="jksec alt"><div class="jkhead"><h2>보건·의료에서 바뀌는 것 <span class="n">공식 출처 · 검토 '+health.length+'건</span></h2><div class="more"><a href="#list" data-ch="health">더보기 ›</a></div></div><div class="jk-grid c3">'+fill(health,6)+'</div></section>'
    +'<section class="jksec"><div class="jkhead"><h2>AI 신호 <span class="n">전망에 반영된 공식 자료 '+ai.length+'건</span></h2><div class="more"><a href="#list" data-ch="ai">더보기 ›</a></div></div><div class="jk-grid c5">'+fill(ai,5)+'</div></section>'
    +'<section class="jksec alt"><div class="jkhead"><h2>오늘의 SQL 5문제 <span class="n">원문 3 + 생성 2</span></h2><div class="more"><a href="#list" data-ch="sql">더보기 ›</a></div></div><div class="jk-grid c5">'+fill(sql,5)+'</div></section>';
  const jf=$('jobfam');if(jf){jf.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;jf.querySelectorAll('button').forEach(x=>x.classList.toggle('on',x===b));const f=JOBFAM.find(x=>x[0]===b.dataset.k)[2];$('jobgrid').innerHTML=fill(jobs.filter(f),8);});}
  rows.addEventListener('click',e=>{const a=e.target.closest('a[data-ch]');if(!a)return;e.preventDefault();const d=$('listtoggle');d.open=true;const li=document.querySelector('#f-channel li[data-v="'+a.dataset.ch+'"]');if(li)li.click();d.scrollIntoView({behavior:'smooth',block:'start'});});
  const ib=document.querySelector('#insight a[data-ch]');if(ib)ib.addEventListener('click',e=>{e.preventDefault();const d=$('listtoggle');d.open=true;const li=document.querySelector('#f-channel li[data-v="health"]');if(li)li.click();d.scrollIntoView({behavior:'smooth'});});
  const I=F.insight||{};
  if(I.headline){$('ins-headline').textContent=I.headline;$('ins-points').innerHTML=(I.points||[]).map(p=>'<li>'+esc(p)+'</li>').join('');if(I.watch_next){$('ins-next').hidden=false;$('ins-next').innerHTML='<b>다음 볼 것</b>'+esc(I.watch_next);}$('ins-date').textContent=I.text_date?'· 분석 '+I.text_date:'';}
  if(I.method)$('ins-method').textContent=I.method;
  const nb=I.new_by_channel||{};const pl=F.pipelines||[];const st={ok:'정상',fail:'실패',none:'없음'};
  $('ins-stats').innerHTML='<div class="it"><div class="v">'+esc(F.meta&&F.meta.today_new||0)+'<small>건</small></div><div class="k">오늘 신규</div></div>'
