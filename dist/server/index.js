...
<section data-view="practice" id="view-practice">
  <div class="wrap pg">
    <div class="prac">
      <div>
        <div class="box"><h2 id="sql-title">오늘의 SQL 5문제</h2><p class="sub" id="sql-sub">요일별 주제 · 원문 링크 3 + 생성 2 · BigQuery·MariaDB 채점</p><div id="sqllist"></div></div>
        <div class="box"><h2>Python 문제</h2><p class="sub">아직 문제 은행이 없습니다. SQL 데일리와 같은 방식(요일별 주제 · 원문 링크 + 생성 문제 · 자동 채점)으로 만들 예정입니다.</p></div>
      </div>
      <aside>
        <div class="box"><h2 style="font-size:15px">푸는 법</h2><ol style="margin:0;padding-left:18px;font-size:14px;color:var(--sec);line-height:1.7"><li>원문 링크 문제는 해당 사이트에서 풀고 <code>/sql done N</code>으로 기록</li><li>생성 문제는 <code>/sql check N &lt;쿼리&gt;</code>로 채점. 정답 행 수와 차이 행이 표시됨</li><li>일요일은 이번 주 오답 복습</li></ol></div>
        <div class="soon" id="sqlstats">풀이 기록이 쌓이면 정답률·연속 일수가 여기에 표시됩니다.</div>
      </aside>
    </div>
  </div>
</section>
...
function renderRows(F){
  ...
  // --- 문제 풀이
  const sql=F.items.filter(i=>i.channel==='sql');
  if($('sqllist')){const sp=pl.find(p=>p.id==='sql');if(sp)$('sql-sub').textContent=(sp.note||'')+' · '+(sp.last_run||'');
    $('sqllist').innerHTML=sql.length?sql.map((i,k)=>'<div class="prow"><div class="num">'+(k+1)+'</div><div><h3>'+(i.url?'<a href="'+esc(i.url)+'" target="_blank" rel="noopener noreferrer">'+esc(i.title)+'</a>':esc(i.title))+'</h3>'+(i.summary?'<div class="d">'+esc(i.summary)+'</div>':'')+'<div class="tg">'+(i.tags||[]).map(t=>'<span>'+esc(t)+'</span>').join('')+'<span>'+esc(i.publisher)+'</span></div></div><div class="st">'+(i.url?'원문':'생성')+'</div></div>').join(''):'<p class="empty">오늘 문제가 없습니다.</p>';}
  // --- 공통: 상태바, 목록 링크
