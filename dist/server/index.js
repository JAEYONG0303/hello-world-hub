...
.wrow{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;align-items:center;padding:8px 18px;border-top:1px solid var(--grid-s)}
...
function renderRows(F){
  ...
  const WI=(F.watch_institutions||[]);let wOff={};try{wOff=JSON.parse(localStorage.getItem('hub.watch.off')||'{}');}catch(e){wOff={};}
  const wOn=w=>(w.name in wOff)?!wOff[w.name]:w.on;
  const instOf=i=>WI.find(w=>w.match.some(m=>(i.publisher||'').includes(m)));
  const drawWatch=()=>{if(!$('winst'))return;const on=WI.filter(wOn).length;
    $('winst').innerHTML='<div class="rail-label" style="margin-top:18px">관심 기관 <span class="wn">'+on+'/'+WI.length+' ON</span></div>'+WI.map(w=>{const o=wOn(w);
      return '<div class="wrow'+(o?'':' off')+'"><button class="wname" data-q="'+esc(w.name)+'" title="이 기관 공고만 보기">'+esc(w.name)+'</button><span class="wopen">'+(w.open?'채용중 <b>'+w.open+'</b>':'<i>0</i>')+(w.soon?' <em>D-7 '+w.soon+'</em>':'')+'</span><span class="wtog" data-name="'+esc(w.name)+'" role="switch" aria-checked="'+o+'"><b>ON</b><b>OFF</b></span></div>';}).join('');};
  if($('winst')){$('winst').addEventListener('click',e=>{const t=e.target.closest('.wtog');if(t){const n=t.dataset.name;const w=WI.find(x=>x.name===n);wOff[n]=wOn(w);try{localStorage.setItem('hub.watch.off',JSON.stringify(wOff));}catch(x){}drawWatch();if(st.watch){jlim=15;drawJobs();}return;}
      const b=e.target.closest('.wname');if(b){$('f-q').value=b.dataset.q.replace(/\\(.*\\)$/,'');st.q=$('f-q').value;jlim=15;drawJobs();window.scrollTo({top:0,behavior:'smooth'});}});}
  ...
  if($('ftool')){$('ftool').addEventListener('click',e=>{const b=e.target.closest('.fchips button');if(!b)return;const k=b.parentElement.dataset.k;st[k]=b.dataset.v;jlim=15;drawJobs();});
    ...
    $('f-watch').addEventListener('click',()=>{st.watch=!st.watch;$('f-watch').classList.toggle('on',st.watch);jlim=15;drawJobs();});
    drawWatch();
    $('f-reset').addEventListener('click',()=>{FK.forEach(k=>st[k]='');st.q='';st.soon=false;st.watch=false;$('f-q').value='';$('f-soon').classList.remove('on');$('f-watch').classList.remove('on');jlim=15;drawJobs();});
    drawJobs();}
