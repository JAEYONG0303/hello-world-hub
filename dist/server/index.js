...
<script id="hub-stats" type="application/json">{"updated":"2026-09-10","jobs":{"total":73,"upcoming7":32,"next_deadline":"2026-09-10"},"sql":{"bank":36,"engines":{"bigquery":18,"mariadb":18},"solved":0,"correct":0,"accuracy":null,"days":0},"star":{"cards":5,"confirmed":0,"open_items":12},"tracker":{"total":0,"by_status":{}},"note":"my agent 저장소의 CSV·YAML·MD 에서 센 값. 추정치 없음."}</script>
...
function renderRows(F){
  ...
  const st={region:'',org_type:'',job_family:'',experience:'',employment:'',source:'',q:'',soon:false};
  ...
  const drawJobs=()=>{const list=jobs.filter(i=>pass(i,null));const shown=list.slice(0,jlim);
    ...
    $('jobs-foot').innerHTML=(list.length>shown.length?'<button class="more" id="jobs-more">'+(list.length-shown.length)+'건 더 보기</button><br><br>':'')+shown.length+'건 표시 · 신입·경력무관 · 마감 가까운 순';
    const mb=$('jobs-more');if(mb)mb.addEventListener('click',()=>{jlim+=15;drawJobs();});drawChips();};
  if($('ftool')){$('ftool').addEventListener('click',e=>{const b=e.target.closest('.fchips button');if(!b)return;const k=b.parentElement.dataset.k;st[k]=b.dataset.v;jlim=15;drawJobs();});
    $('f-q').addEventListener('input',e=>{st.q=e.target.value.trim();jlim=15;drawJobs();});
    $('f-soon').addEventListener('click',()=>{st.soon=!st.soon;$('f-soon').classList.toggle('on',st.soon);jlim=15;drawJobs();});
    $('f-reset').addEventListener('click',()=>{FK.forEach(k=>st[k]='');st.q='';st.soon=false;$('f-q').value='';$('f-soon').classList.remove('on');jlim=15;drawJobs();});
    drawJobs();}
